...
const SyntaxHighlighter = dynamic<SyntaxHighlighterProps>(
   () => import('react-syntax-highlighter').then((m) => m.Prism as never),
   {
      ...
      loading: ({ error }) =>
         error ? null : (
            <pre className="m-0 p-4 text-[1em] bg-gray-900 overflow-x-auto" />
         ),
   },
);
...
export function CodeBlock({ inline, className, children }: CodeBlockProps) {
   ...
   if (inline || !language) {
      return (
         <code className="bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded px-1 py-0.5 text-[1em] font-mono">
            {children}
         </code>
      );
   }
